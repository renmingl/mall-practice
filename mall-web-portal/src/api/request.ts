...
service.interceptors.response.use(
  ...
  async (error: AxiosError<ApiResult>) => {
    ...
    // 登录/注册/刷新接口自身 401 不重试（登录失败与 refresh 失效直接走清态逻辑）
    const noRetry = config?.url?.includes('/auth/login') || config?.url?.includes('/auth/register') || config?.url?.includes('/auth/refresh')
